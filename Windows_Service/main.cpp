...
VOID WINAPI SVC_MAIN( DWORD dw, LPWSTR * lp)
{
    ...
    status_handle=RegisterServiceCtrlHandlerW(SVCNAME, (LPHANDLER_FUNCTION)Ctrl_HL);
    if(!status_handle)
    {
    ...
    status.dwControlsAccepted = SERVICE_ACCEPT_STOP | SERVICE_ACCEPT_POWEREVENT | SERVICE_ACCEPT_SESSIONCHANGE | SERVICE_ACCEPT_SHUTDOWN;
...
void init()
{
    ...
    QObject::connect(w, &worker::quit, [&](){
        a.quit();});
    a.exec();
}

DWORD WINAPI Ctrl_HL(DWORD control)
{
    switch (control)
    {
